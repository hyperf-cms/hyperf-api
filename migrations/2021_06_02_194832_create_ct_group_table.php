...
class CreateCtGroupTable extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('ct_group', function (Blueprint $table) {
            ...
            $table->integer('uid')->default('0')->comment('创建群用户ID');
            $table->string('group_name', 30)->default('0')->comment('群名称');
            $table->string('avatar', 255)->default('')->comment('头像');
            $table->integer('size')->default('200')->comment('群规模 200 500 1000');
        ...
        \Hyperf\DbConnection\Db::statement("ALTER TABLE `".(env('DB_PREFIX','')?:'')."ct_group` comment'群组表'");//表注释一定加上前缀
    }
