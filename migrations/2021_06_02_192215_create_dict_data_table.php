...
class CreateDictDataTable extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('dict_data', function (Blueprint $table) {
            ...
            $table->string('dict_label', 100)->default('')->comment('字典标签');
            $table->string('dict_value', 100)->default('')->comment('字典键值');
            $table->string('dict_type', 100)->default('')->comment('字典类型');
            $table->string('css_class', 100)->default('')->comment('样式属性（其他样式扩展）');
            $table->string('list_class', 100)->default('')->comment('表格回显样式');
