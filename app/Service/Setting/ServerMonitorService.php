...
class ServerMonitorService extends BaseService
{
    ...
    public function getCpuInfo() : array
    {
        ...
        if(false !== is_array($model[1])){
            ...
            $x1 = ($cpuInfo['cpu']['num' ]== 1) ? '' : '×' . $cpuInfo['cpu']['num'];
            $mhz[1][0] = '|频率:' . $mhz[1][0];
            $cache[1][0] = '|二级缓存:' . $cache[1][0];
            $boGomIps[1][0] = '|Bogomips:' . $boGomIps[1][0];
            $cpuInfo['cpu']['model'][] = $model[1][0].$mhz[1][0] . $cache[1][0] . $boGomIps[1][0] . $x1;
            if(false !== is_array($cpuInfo['cpu']['model'])) $cpuInfo['cpu']['model'] = implode("<br/>", $cpuInfo['cpu']['model']);
            if(false !== is_array($cpuInfo['cpu']['mhz'])) $cpuInfo['cpu']['mhz'] = implode("<br/>", $cpuInfo['cpu']['mhz']);
            if(false !== is_array($cpuInfo['cpu']['cache'])) $cpuInfo['cpu']['cache'] = implode("<br/>", $cpuInfo['cpu']['cache']);
            if(false !== is_array($cpuInfo['cpu']['bogomips'])) $cpuInfo['cpu']['bogomips'] = implode("<br/>", $cpuInfo['cpu']['bogomips']);
        }
        return $cpuInfo;
    }
